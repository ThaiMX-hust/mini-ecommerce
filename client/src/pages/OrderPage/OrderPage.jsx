...
const OrderPage = () => {
    ...
    const [paymentModal, setPaymentModal] = useState({ isOpen: false, order: null, isLoading: false });
    ...
    const handlePaymentConfirm = async (paymentMethod) => {
        ...
        try {
            if (paymentMethod === 'vnpay') {
                const paymentRes = await api.post("/payments/vnpay", {
                    orderId: paymentModal.order.order_id, 
                });
                ...
                // Stripe trả về clientSecret, KHÔNG phải url
                const paymentRes = await api.post("/payments/stripe", {
                    orderId: paymentModal.order.order_id
                });

                // Đóng modal payment method
                setPaymentModal({ isOpen: false, order: null, isLoading: false });
                
                // Hiển thị Stripe checkout form
                setCurrentOrderId(paymentModal.order.order_id);
    ...
    const handleStripeSuccess = () => {
        ...
        fetchOrderHistory(); // Refresh orders
        ...
        return (
            <div className={styles.container}>
                <div className={styles.stripeFormContainer}>
                    <h2 className={styles.stripeTitle}>Thanh toán qua Stripe</h2>
                    <p className={styles.orderInfo}>Mã đơn hàng: {currentOrderId}</p>
